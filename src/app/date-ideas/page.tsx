    ...
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(n => (
        <button key={n} onClick={() => onRate(n)}
          className={`text-sm transition-colors ${n <= rating ? 'text-[#C4784A]' : 'text-[#E8DDD4] hover:text-[#C4784A]/50'}`}>
          ♥
        </button>
      ))}
    </div>
...
export default function DateIdeasPage() {
  ...
  async function addIdea(e: React.FormEvent) {
    ...
    const { data } = await supabase
      .from('date_ideas')
      .insert({ title: title.trim(), category, done: false, rating: 0, notes: notes.trim() || null })
      .select()
      .single()
  ...
  const filtered = ideas.filter(i =>
    filter === 'all' ? true : filter === 'todo' ? !i.done : i.done
  )
  ...
  return (
    <div className="pt-20 p-6 md:p-10 max-w-2xl">
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="text-[#7A6155] mt-1 text-sm">
            {ideas.filter(i => i.done).length} done · {ideas.filter(i => !i.done).length} ideas left
          </p>
        </div>
        <button onClick={() => setShowForm(s => !s)}
          className="px-4 py-2 rounded-xl text-sm font-medium bg-[#C4784A] text-white hover:bg-[#B36840] transition-colors">
          + Add
        </button>
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        {filtered.map(idea => (
          <div key={idea.id}
            className={`rounded-2xl border border-[#E8DDD4] bg-white p-4 flex flex-col gap-2 group ${idea.done ? 'opacity-60' : ''}`}
            style={{ boxShadow: '0 2px 12px rgba(44,26,14,0.05)' }}>
            <div className="flex items-start gap-3">
              <button onClick={() => toggleDone(idea.id, idea.done)} className="shrink-0 mt-0.5">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-xs text-[#AE9B8E]">
                    {categories[idea.category as Category]?.emoji} {categories[idea.category as Category]?.label}
                  </span>
                </div>
