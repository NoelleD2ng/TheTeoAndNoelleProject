    ...
    <div className={`${border} rounded-2xl p-4 group flex flex-col gap-2`} style={card}>
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm text-white/95 font-medium flex items-center gap-1.5">
              {place.name}
              {place.visited && <span className="text-[#c8a97e] text-xs">✓</span>}
            </p>
            {place.location && <p className="text-xs text-white/55">{place.location}</p>}
          </div>
        </div>
        <button onClick={() => remove(place.id)} className="opacity-0 group-hover:opacity-100 text-white/40 hover:text-white/75 text-xs transition-all">✕</button>
      </div>
      {place.notes && <p className="text-xs text-white/55 pl-10">{place.notes}</p>}
      {!place.visited && (
        <button onClick={() => toggleVisited(place.id)} className="text-xs text-white/40 hover:text-[#c8a97e]/60 transition-colors text-left pl-10">mark visited ✓</button>
      )}
    </div>
  )
    ...
    <div className="pt-20 p-6 md:p-10">
      <div className="mb-8 flex items-start justify-between">
        <div>
          ...
          <p className="text-white/60 mt-1 text-sm">places we want to go and places we&apos;ve been</p>
        </div>
      ...
      {wantToGo.length > 0 && (
        <div className="mb-8">
          <p className="text-[10px] tracking-[0.25em] uppercase text-white/45 mb-3">Want to go</p>
      ...
      {beenThere.length > 0 && (
        <div>
          <p className="text-[10px] tracking-[0.25em] uppercase text-white/45 mb-3">Been there</p>
